...
def video_frame_callback(frame: av.VideoFrame) -> av.VideoFrame:
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        ...
        if score < 0.5:
            cv2.putText(img, "Hand detection too weak!", (30, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        elif not is_landmarks_valid(lms):
            cv2.putText(img, "Invalid hand shape!", (30, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        else:
            ...
            if conf < 0.8:
                cv2.putText(img, "Low prediction confidence!", (30, 90),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            else:
                predicted_letter = labels[pred_idx]
                cv2.putText(img, f"Predicted: {predicted_letter} ({conf:.2f})", (30, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    else:
        cv2.putText(img, "No hand detected", (30, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
webrtc_streamer(
    ...
    media_stream_constraints={
        "video": {
            "facingMode": "environment"  # <-- select rear camera on mobile devices
        },
        "audio": False  # Disable microphone capture as you did
    }
)
